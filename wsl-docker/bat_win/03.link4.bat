...
IF "%_DOMAIN_ID4_%"=="" (
	GOTO STDIN_DOMAIN_ID4_
)
GOTO STDINPASS_DOMAIN_ID4_
:STDIN_DOMAIN_ID4_
SET /p _DOMAIN_ID4_=https접속할 도메인 앞자리(prefix-ID)를 입력하세요:
:STDINPASS_DOMAIN_ID4_


:: devbox.firstmall.kr-api id 확인/포트확인 > curl "https://devbox.firstmall.kr/?action=add&id=DOMAINID" 
:: 실행 명령어 저장 
FOR /f "delims=" %%A IN ('curl "https://%_DEV_SERVER_%/?action=add&id=%_DOMAIN_ID4_%"') DO (SET PORT=%%A)
SET PORT=%PORT:-=%
SET PORT=%PORT: =%
echo "PORT=%PORT%"

echo "비밀번호를 정확하게 입력한 뒤 [https://%_DOMAIN_ID4_%.%_DEV_SERVER_%]으로 접속 가능합니다."

:: 리버스터널링 접속 ex> ssh -NR 24080:localhost:8880 gs1902841@192.168.243.28
ssh -NR %PORT%:localhost:%_LOCAL_PORT4_% %_IDNUM_%@%_DEV_SERVERIP_%
